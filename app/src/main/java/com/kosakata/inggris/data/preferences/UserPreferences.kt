/*
 * Tujuan: Menyimpan konfigurasi belajar/audio dan statistik ringan pengguna.
 * Caller: Screen Compose dan VocabRepository.
 * Dependensi: Android DataStore Preferences dan timezone perangkat.
 * Main Functions: Flow preferences, setter tervalidasi, recordStudyDay, recordWordsHeard.
 * Side Effects: Membaca dan menulis DataStore user_preferences.
 */
...
class UserPreferences(private val context: Context) {
    private object Keys {
        ...
        val WORDS_HEARD = intPreferencesKey("words_heard")
    ...
    val wordsHeard: Flow<Int> = context.dataStore.data.map { it[Keys.WORDS_HEARD] ?: 0 }
    ...
    suspend fun setRepeatCount(value: Int) =
        context.dataStore.edit { it[Keys.REPEAT_COUNT] = value.coerceIn(1, 100) }
    suspend fun setListeningDelay(value: Int) =
        context.dataStore.edit { it[Keys.LISTENING_DELAY] = value.coerceIn(0, 60) }
    ...
    suspend fun setStreak(count: Int, lastStudyDate: Long) = context.dataStore.edit {
        ...
        it[Keys.LONGEST_STREAK] = maxOf(it[Keys.LONGEST_STREAK] ?: 0, count)
        it[Keys.LAST_STUDY_DATE] = lastStudyDate
    }

    suspend fun recordWordsHeard(count: Int = 1) = context.dataStore.edit {
        it[Keys.WORDS_HEARD] = (it[Keys.WORDS_HEARD] ?: 0) + count.coerceAtLeast(0)
    }

    suspend fun resetLearningStats() = context.dataStore.edit {
        it[Keys.STREAK_COUNT] = 0
        it[Keys.LONGEST_STREAK] = 0
        it[Keys.LAST_STUDY_DATE] = 0L
        it[Keys.WORDS_HEARD] = 0
    }
